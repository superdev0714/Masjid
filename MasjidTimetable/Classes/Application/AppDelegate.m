...
@interface AppDelegate () <AVAudioPlayerDelegate, UNUserNotificationCenterDelegate>
{
...
//    localNotification = [UILocalNotification new]; // TODO: notification upgrade
    ...
    (void)(val=1),(void)(val1=1),(void)(val2=1),(void)(val3=1),(void)(val4=1),(void)(val5=1),(void)(val6=1),alert=0;
...
{
    // Vimal- 11/10/2017 Local Notification
...
//    
//    /*NSMutableArray *fireDateInfo = [[NSMutableArray alloc]init];
//    [fireDateInfo addObject:@"2017-10-08 11:02:00 +0000"];
//    [fireDateInfo addObject:@"2"];
//    [fireDateInfo addObject:@"11:02"];*/
//    
//    
//    
//    if (IS_IOS_10_OR_LATER) {
...
//        
//        //Vimal Local Notification-Final earlier code
...
//                //NSLog(@"%@", content.body);
//                /*UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Sample Alert"
...
- (NSArray*)getFireDateInfoWithIdentifier:(NSString *)identifier dayIndex:(int)dayIndex
{
  ...
  if ([identifier hasSuffix:@"B"]) {
    ...
    //VImal Notification testing
   if ([identifier isEqualToString:@"zoharB"]) {
       //fajarTime = @"11.02";
       //isTimeFromatAM = YES;
       fajarTime =  [Utils reformatTimeTo24timeFormat:fajarTime isAMformat:YES withTimeFormat:tableTimeFormat.format];
   }
   else{
  ...
  NSLog(@">>>> event date: %@, userSettedTime: %@", jamatDate, userSettedTimeCount);
    ...
    jamatDate = [jamatDate dateByAddingTimeInterval: (-60 * [userSettedTimeCount integerValue])];//-60
            ...
            [center getPendingNotificationRequestsWithCompletionHandler:^(NSArray<UNNotificationRequest *> * _Nonnull requests) {
              for (UNNotificationRequest *request in requests) {
                  if ([request.identifier isEqualToString:identifier]) {
                      ...
                      [center addNotificationRequest:request withCompletionHandler:^(NSError * _Nullable error) {
                          if (!error) {
                              NSLog(@">>>>>>> add NotificationRequest succeeded!-5");
                              NSLog(@"Sample Testing----------");
                          }
                      }];
...
/*- (void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void(^)(void))completionHandler {
    ...
    if ([response.notification.request.identifier isEqualToString:@"Alert"]) {
...
- (void)sampleDemoApp:(NSArray*)info {
    ...
    UNNotificationCategory *category = [UNNotificationCategory categoryWithIdentifier:@"Alert"
    ...
    ////Vimal -local notification
    NSDate *fireDate = [[NSDate alloc] initWithTimeInterval:1 sinceDate:fireDate1];//1
    ...
    NSString *identifier1 = @"Alert";
    UNNotificationRequest *request = [UNNotificationRequest requestWithIdentifier:identifier1 content:content trigger:trigger];
